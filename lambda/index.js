...
const LaunchRequestHandler = {
  ...
  async handle(handlerInput) {
    ...
    // To do: Allow user to specify the year; add slots and mp3 consts to DynamoDB?
    if (!playbackInfo.hasPreviousPlaybackSession) {
      message = 'Welcome to Arcade Party. To start listening to arcade sounds from 1981, say, Play!';
      reprompt = 'Do you want to play arcade sounds?';
    } else {
      message = `You were listening to an ${constants.audioData[playbackInfo.playOrder[playbackInfo.index]].title}. Would you like to resume?`;
...
const HelpHandler = {
  ...
  async handle(handlerInput) {
    ...
    if (!playbackInfo.hasPreviousPlaybackSession) {
      message = 'Welcome to Arcade Party. Would you like to be audibly transported to an arcade in 1981, 1983, 1986, or 1992?';
    } else if (!playbackInfo.inPlaybackSession) {
      message = `You were listening to an ${constants.audioData[playbackInfo.index].title}. Would you like to resume?`;
    } else {
      message = 'Welcome back to Arcade Party. You can say Next or Previous to navigate through the playlist. At any time, you can say Pause to pause arcade sounds and Resume to resume.';
    }
...
const ExitHandler = {
  ...
  handle(handlerInput) {
    return handlerInput.responseBuilder
      .speak('Thanks for listening to Arcade Party! Thanks to Andy Hofle for making these sounds and the Internet Archive for hosting them! Goodbye!')
      .getResponse();
  },
};
...
const ErrorHandler = {
  ...
  handle(handlerInput, error) {
    ...
    const message = "Sorry, I don't understand that command. Please say help to hear what you can say.";
...
const controller = {
  async play(handlerInput) {
    ...
    responseBuilder
      .speak(`You are listening to the ${podcast.title}. To navigate to other years, say Next or Previous.`)
      .withShouldEndSession(true)
  ...
  // To-do: We want to likely loop or ask for a new year on Next/Previous walls
  async playNext(handlerInput) {
    ...
    if (nextIndex === 0 && !playbackSetting.loop) {
      return handlerInput.responseBuilder
        .speak('You have reached the end of Arcade Party! Thanks for listening and thanks to Andy Hofle for making the Arcade Ambience tracks and the Internet Archive for hosting them! Say Restart or Previous to listen to more.')
        .addAudioPlayerStopDirective()
        .getResponse();
    }
  ...
  async playPrevious(handlerInput) {
    ...
    if (previousIndex === -1) {
      if (playbackSetting.loop) {
        ...
        return handlerInput.responseBuilder
          .speak('You have reached the first Arcade Party track, from 1981!')
          .addAudioPlayerStopDirective()
          .getResponse();
      }
    }
